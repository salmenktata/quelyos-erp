...
def find_faq_match(message: str, threshold: float = 0.5) -> Optional[Dict]:
    ...
    Returns:
        dict: Entrée FAQ correspondante ou None
        {
            'question': str,
            'answer': str,
            'score': float,
    ...
    for entry in FAQ_ENTRIES:
        ...
        if score > best_score and score >= threshold:
            ...
            best_match = {
                ...
                'answer': entry['answer'],
                'score': score,
                'is_exact': score >= 0.8  # Match très fort
            }
...
def get_faq_response(message: str) -> Tuple[Optional[str], Optional[Dict]]:
    ...
    Returns:
        Tuple[Optional[str], Optional[Dict]]: (réponse, metadata)
    """
    ...
    if match and match['score'] >= 0.5:
        ...
        # Ajouter disclaimer si match pas parfait
        if match['score'] < 0.8:
