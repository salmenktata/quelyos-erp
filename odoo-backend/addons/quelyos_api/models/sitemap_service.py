...
class SitemapService(models.AbstractModel):
    ...
    def get_sitemap_cached(self):
        """
        Récupère sitemap depuis cache Redis ou régénère.
        Cache TTL: 5 minutes.
        """
        # Essayer cache Redis
        cache = self.env['quelyos.cache'].sudo()
        cached_data = cache.get_json(self.CACHE_KEY)

        if cached_data:
            _logger.info('[Sitemap] Serving from Redis cache')
            return cached_data

        # Régénérer sitemap
        _logger.info('[Sitemap] Cache miss, generating sitemap...')
        sitemap_data = self._generate_sitemap()

        # Mettre en cache avec TTL 5 min
        cache.set_json(self.CACHE_KEY, sitemap_data, ttl=self.CACHE_TTL)

        return sitemap_data

    def _generate_sitemap(self):
        """
        Génère sitemap en lisant fichier JSON ou via script TypeScript.

        Stratégie:
        1. Lire fichier JSON pré-généré (sitemap.json)
        2. Si absent, exécuter script generate-sitemap.ts --json
        3. Si échec, fallback structure vide
        """
        try:
            # Chemin vers fichier JSON
            json_file = '/mnt/extra-addons/quelyos_api/data/sitemap.json'

            # Essayer de lire fichier JSON pré-généré
            if subprocess.os.path.exists(json_file):
                _logger.info('[Sitemap] Reading from sitemap.json')
                with open(json_file, 'r') as f:
                    sitemap = json.load(f)

                # Enrichir avec healthcheck
                sitemap = self._enrich_with_health(sitemap)
                return sitemap

            # Sinon, exécuter script TypeScript
            _logger.info('[Sitemap] Generating via TypeScript script...')
    ...
    def trigger_healthcheck(self, app_id=None):
        ...
        Exécution async via job queue.

        Args:
            app_id (str): ID app à checker, ou None pour toutes

        Returns:
            dict: Info sur le job créé
        """
        # Récupérer sitemap
    ...
    def _run_healthcheck_job(self, apps):
        ...
        # Invalider cache pour forcer refresh avec nouvelles données healthcheck
        cache = self.env['quelyos.cache'].sudo()
        cache.delete(self.CACHE_KEY)
        _logger.info('[Sitemap] Cache invalidated after healthcheck')
